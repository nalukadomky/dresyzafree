...
const mapMatch = (row: any): Match => ({
  ...
  matchRating: row.match_rating ?? undefined,
  createdAt: row.created_at,
});
...
export const dbPlayers = {
  ...
  matches: {
    ...
    update: async (
      matchId: string,
      teamId: string,
      updates: { result?: string; goalsFor?: number; goalsAgainst?: number; matchRating?: number }
    ): Promise<Match | null> => {
      const toUpdate: Record<string, unknown> = {};
      ...
      if ('matchRating' in updates) toUpdate.match_rating = updates.matchRating ?? null;
      if (Object.keys(toUpdate).length === 0) return null;
      const { data, error } = await client!
        .from('matches')
        .update(toUpdate)
